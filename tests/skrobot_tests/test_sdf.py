...
class TestSDF(unittest.TestCase):
    ...
    def setup_class(cls):
        ...
        # prepare PointCloudSDF
        cls.cloud_radius = 0.05
        cls.cloud_points = np.random.rand(100, 3)
        cls.cloud_sdf = PointCloudSDF(cls.cloud_points, cls.cloud_radius)
    ...
    def test_pointcloudsdf__signed_distance(self):
        queries = np.random.rand(100, 3)
        values = self.cloud_sdf(queries)

        # compare with ground truth generated by brute force
        gt_values = []
        for q in queries:
            dists = np.linalg.norm(self.cloud_points - q, axis=1)
            gt_values.append(np.min(dists) - self.cloud_radius)
        testing.assert_almost_equal(values, gt_values)
